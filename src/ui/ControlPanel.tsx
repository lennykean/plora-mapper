import { Group, Select, MultiSelect, SegmentedControl, Button, Text } from "@mantine/core";
...
export default function ControlPanel() {
  ...
  function setQualifiers(values: string[]) {
    ...
    updateOptions({ ...state.options, preferQualifier: merged.length ? merged : undefined });
  }

  function setExcludeQualifiers(values: string[]) {
    const stressExcludes = (state.options.preferQualifierNot ?? []).filter((q) =>
      ["stressed", "unstressed"].includes(q)
    );
    const merged = [...new Set([...stressExcludes, ...values])];
    updateOptions({ ...state.options, preferQualifierNot: merged.length ? merged : undefined });
  }

  function setStress(value: string) {
    const opts = { ...state.options };
    if (value === "weak") {
      opts.preferQualifier = [...(opts.preferQualifier ?? []).filter((q) => q !== "strong form" && q !== "stressed"), "weak form", "unstressed"];
      opts.preferQualifierNot = [...(opts.preferQualifierNot ?? []).filter((q) => q !== "unstressed"), "stressed"];
    } else if (value === "strong") {
      opts.preferQualifier = [...(opts.preferQualifier ?? []).filter((q) => q !== "weak form" && q !== "unstressed"), "strong form", "stressed"];
      opts.preferQualifierNot = [...(opts.preferQualifierNot ?? []).filter((q) => q !== "stressed"), "unstressed"];
    } else {
      opts.preferQualifier = (opts.preferQualifier ?? []).filter((q) => !["weak form", "unstressed", "strong form", "stressed"].includes(q));
      opts.preferQualifierNot = (opts.preferQualifierNot ?? []).filter((q) => !["stressed", "unstressed"].includes(q));
    ...
    <Group gap="sm" mt="xs" wrap="wrap" justify="space-between">
      ...
      {state.availableQualifiers.length > 0 && (
        <MultiSelect
          ...
          value={(state.options.preferQualifier ?? []).filter((q) => !["weak form", "unstressed", "strong form", "stressed"].includes(q))}
      ...
      {state.availableQualifiers.length > 0 && (
        <MultiSelect
          ...
          data={state.availableQualifiers}
          value={(state.options.preferQualifierNot ?? []).filter((q) => !["stressed", "unstressed"].includes(q))}
      ...
      <Group gap={4}>
        <Text size="xs" c="dimmed">Stress</Text>
        <SegmentedControl
          size="xs"
          data={[
            { label: "Off", value: "off" },
